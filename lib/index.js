...
/**
 * The session's complete event log across both host generations: 0.1.5 exposes
 * the immutable `snapshotEvents()` read, older hosts the `events` array.
 * `undefined` means "not a live session", which is what the guard reports.
 */
...
function assertSession(session) {
  if (session === null || typeof session !== 'object' || typeof session.append !== 'function' || sessionEvents(session) === undefined) {
...
export function createWorkflowConfiguration(options = {}) {
  const readState = options.readState ?? ((session) => foldModeState(sessionEvents(session) ?? []))
